...
class lfc_rule(SetRule):
    ...
    def evaluate(self, sub_grid, grid, pencil_grid):
        candidates = sub_grid.candidates()

        if len(candidates) == 1:
            coord = sub_grid.coord(sub_grid.incomplete_cells()[0])
            grid.update_cell(coord, candidates[0], pencil_grid)
...
class ns_rule(CellRule):
    ...
    def evaluate(self, coord, grid, pencil_grid):
        candidates = pencil_grid.cell(coord)
...
class uc_rule(CellRule):
    ...
    def evaluate(self, coord, grid, pencil_grid):
        ...
        row_sg_pencil = self.cover_and_check(coord, grid, pencil_grid, SG_Type.ROW)
        
        col_sg_pencil = self.cover_and_check(coord, grid, pencil_grid, SG_Type.COLUMN)
        
        sqr_sg_pencil = self.cover_and_check(coord, grid, pencil_grid, SG_Type.SQUARE)
        ...
        for candidate in pencil:
            if candidate not in row_sg_pencil or candidate not in col_sg_pencil or candidate not in sqr_sg_pencil:
                grid.update_cell(coord, candidate, pencil_grid)
